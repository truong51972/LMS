...
def course_delete(request, course_pk):
    ...
    context = {"name": course.course_name, "cancel_link": reverse("course:course_list")}
...
def course_edit(request, course_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                reverse("course:course_view", kwargs={"course_pk": course_pk})
            )
    else:
...
def upload_existed_course(request):
    ...
    if request.method == "POST":
        ...
        for course_name, course_items in json_data.items():
            ...
            for sub_course_title, sub_course_items in course_items[
                "sub_courses"
            ].items():
                order = len(course.sub_courses.all()) + 1
                sub_course, _ = Sub_Course.objects.get_or_create(
                    title=sub_course_title, course=course, defaults={"order": order}
                )
                for module_title, module_items in sub_course_items["modules"].items():
                    ...
                    module, _ = Module.objects.get_or_create(
                        ...
                        defaults={"order": order, "created_by": request.user},
                    )
                ...
                for quiz_title, quiz_items in sub_course_items["quizzes"].items():
                    ...
                    for question_text, question_items in quiz_items[
                        "questions"
                    ].items():
                        ...
                        for option_text, is_correct in question_items[
                            "answers"
                        ].items():
